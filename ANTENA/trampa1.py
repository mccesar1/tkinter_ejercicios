...
#######Funcion para crear la base de datos #############################################################################################################
  def create_database(self):
    try:
        self.conn = sqlite3.connect('trampa.db')
        self.cursor = self.conn.cursor()
        self.cursor.execute('CREATE TABLE IF NOT EXISTS datos2 (peso INTEGER, edad INTEGER, sexo TEXT, arete2 TEXT)')
        self.conn.commit()
  ...
  def conexion(self, query, parameters):
     try:
      ...
      with sqlite3.connect(self.db_name) as conn:
        ...
        #cursor.execute(query, (parameters,))
        conn.commit()
        messagebox.showinfo("Base de datos", "Datos guardados con exito")
...
######funcion para guardar los datos en la base de datos###############################################################################################        
  ...
  def GuardarEnBD(self):
      try:
        ...
        parameters = (self.Epeso.get(), self.Eedad.get(), self.cmb_sexo.get(), self.Earete.get())
  ...
  def GetData(self):
        ...
        while True:
         ...
         if data:
            ...
            #  self.lista2 = self.grid3
            #  #self.lista2.append(self.grid3)########  cuidado

            #  print (self.lista2)

             #  self.grid3.get_children()
...
#         sheet['D1'] = 'Peso' #se le asigna un titulo a la columna D
#         sheet['E1'] = 'Edad' #se le asigna un titulo a la columna E
#         sheet['F1'] = 'Sexo' #se le asigna un titulo a la columna F
#         #se le asigna un valor a cada celda
#         for i in range(1, len(self.lista)+1): 
#             sheet.cell(row=i, column=1).value = self.Earete.get()# i-2 para que no se repitan los datos
#             sheet.cell(row=i, column=2).value = time.strftime("%d/%m/%Y")
#             sheet.cell(row=i, column=3).value = time.strftime("%H:%M:%S")
#             sheet.cell(row=i, column=4).value = self.Epeso.get()
#             sheet.cell(row=i, column=5).value = self.Eedad.get()
#             sheet.cell(row=i, column=6).value = self.cmb_sexo.get()
...
########FRAME DE LA DERECHA##############################################################################################
    ...
    Bxls = Button(right_frame, text='XLS', font=('ARIAL', 24), background='#DCE1D8', command=self.dbToExcel, border=6, highlightthickness=3, activebackground='#DCE1D8', borderwidth=6)
...
####tab6#######################################################################################################################
    ...
    self.Larete = Label(tab6, text="Arete", font=('ARIAL', 28), background='#003C78', foreground='#ffffff', justify='left')
    ...
    self.cmb_usb=ttk.Combobox(tab6, width=10, font=('ARIAL', 28), justify='left', values=[ 'C:', 'D:', 'E:', 'F:', 'G:', 'H:', 'I:'], state='readonly')
    self.cmb_usb.place(x=290, y=380, width=200, height=60)
    self.cmb_usb.current(1)
    self.btnguardar = Button(tab6, text="Guardar", font=('ARIAL', 28), background='#DCE1D8', command=self.GuardarEnBD, border=6, highlightthickness=3, activebackground='#DCE1D8', borderwidth=6)
    self.btnguardar.place(x=290, y=50, width=200, height=120)
####tab5###########################################################################################
